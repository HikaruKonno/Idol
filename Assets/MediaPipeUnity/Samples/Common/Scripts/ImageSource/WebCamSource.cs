...
#pragma warning restore IDE0025
    ...
    private IEnumerator WaitForCameraConnection()// 奥山：追加↓
    {
    // もしカメラデバイスがすでに選択されていれば、何もしない
    if (webCamDevice != null)
    {
        yield break;
    }

    // WebCamTexture.devices を使ってカメラデバイスを確認
    while (true)
    {
        ...
        {
            webCamDevice = availableSources[0];  // 最初のカメラを選択
            //Debug.Log("Camera connected: " + webCamDevice.name);
            break; // カメラが接続されたのでループを抜ける
        }

        // カメラが接続されていない場合、少し待機してから再確認
        Debug.Log("Waiting for camera connection...");
        ...
        private IEnumerator Initialize()
    {
      ...
      yield return WaitForCameraConnection();       // 奥山：追加
    ...
    private void InitializeWebCamTexture()
    {
      ...
      if (webCamDevice is WebCamDevice valueOfWebCamDevice)
      {
        webCamTexture = new WebCamTexture(valueOfWebCamDevice.name, resolution.width, resolution.height, (int)resolution.frameRate);
                Debug.Log("sssss" + (int)resolution.frameRate);
        return;
      }
