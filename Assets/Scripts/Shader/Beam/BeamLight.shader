...
Shader "HDRP/BeamLight"
{
    Properties
    {
        ...
        [Header(Size)]         // コーンの大きさ
        _ConeWidth ("Cone Width", Range(-0.07, 0.5)) = 0.1
        _ConeLength ("Cone Length", Range(0.01, 3.0)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // _DEPTHFADE_ONというキーワードが有効な時だけ、その機能を持つシェーダーバリアントをコンパイルする
            ...
            Varyings vert(Attributes v)
            {
                ...
                float expansionAmount = (1.0 - v.uv.y) * _ConeWidth;

                // 計算した量だけ、法線方向に頂点を押し出して円錐形にする
                v.positionOS.xyz += v.normalOS.xyz * expansionAmount;

                // Y軸（縦方向）にスケールを掛けて長さを調整する
                v.positionOS.y *= _ConeLength;
            ...
            // フラグメントシェーダー
            half4 frag(Varyings i, bool isFrontFace : VFACE) : SV_Target
            {
                float3 normal = normalize(i.normalWS);
                float3 viewDir = normalize(_WorldSpaceCameraPos - i.positionWS);

                // ビームの基本形状と柔らかさ
                // 光源からの距離減衰を表現し、先端が一番明るくなるようにする
                float grad = 0.1 / distance(i.uv.y, 1.0);
                ...
                // 最終的な色を計算
                half3 baseColor = _Color.rgb * _Intensity * divide * grad * mask * rim * noise * pow(1.5, -_ConeWidth * 20);
