 ...
 * URPからHDRPに変更した為、未使用
 */
Shader "Universal Render Pipeline/ToonURP"
{
    Properties
    {
        [Header(Base)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // URP用のシェーダーバリアントをコンパイル
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS _MAIN_LIGHT_SHADOWS_CASCADE
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            #pragma multi_compile _ _SHADOWS_SOFT

            #pragma shader_feature_local _USE_FACE_MODE
            #pragma shader_feature_local _USE_EYE_MODE
            #pragma shader_feature_local _USE_CLOTH_MODE
            #pragma shader_feature_local _USE_HAIR_MODE
            ...
            half4 frag(Varyings i, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                float lit = 0;
                half3 accumulatedLightColor = mainLight.color;
                half3 highlight = 0;
                ...
                half3 skinShade = 0;
                    ...
                    // // stepでハイライトをくっきりさせる
                    // half3 highlight = step(_HighlighThreshold, specFactor) * _HighlightColor.rgb * mainLight.color;
                    // // 影の中ではハイライトが出ないようにする
                    // highlight *= mainLight.shadowAttenuation;

                    // ハイライトをくっきりさせない版
                    // float highlightDot = 1.0 - dot(normalDir, normalize(viewDir));
                    // float highlightFactor = smoothstep(_HighlighThreshold - _HighlighSmoothness, _HighlighThreshold + _HighlighSmoothness, highlightDot);
                    // half3 highlight = highlightFactor * _HighlightColor.rgb;
                    float3 halfDir = normalize(lightDir + viewDir);
                    // 法線とハーフベクトルの内積を取り、光沢度の鋭さを調整
                    float specFactor = pow(saturate(dot(normalDir, halfDir)), _SpecularGloss);
                    ...
                    // ハイライトの色を決定
                    highlight = highlightFactor * _HighlightColor.rgb * mainLight.color;
                    // 影の中ではハイライトが出ないようにする
                    highlight *= mainLight.shadowAttenuation;
                #endif
                ...
                half3 mainLightColor = mainLight.color * shade;
                ...
                // 影の部分に近いほど環境光を弱くする
                ambient *= lerp(_ShadeDark.rgb, mainLightColor, shadeFactor2);

                // --- 最終的な色を合成 ---
                half3 finalColor = baseColor * (shade * accumulatedLightColor + ambient) + highlight + rimLight + skinShade;
