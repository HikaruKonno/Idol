 ...
 * 
 * 説明
 * HDRPのPBRシェーダーに発光を追加したシェーダー
 * 現在使用しているオブジェクトはない
 */
Shader "HDRP/Emissive"
{
    Properties
    {
        _BaseColor ("Base Color", Color) = (0.1, 0.1, 0.1, 1)
        _EmissiveTex ("Emissive Texture", 2D) = "white" {}
        [HDR] _EmissiveIntensity ("Emissive Intensity", Range(1, 20)) = 5.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                OUT.uv = TRANSFORM_TEX(IN.uv, _EmissiveTex);
            ...
            {
                // テクスチャカラー取得
                half3 texColor = SAMPLE_TEXTURE2D(_EmissiveTex, sampler_EmissiveTex, IN.uv).rgb;

                // 発行色を計算する
                half3 emission = _BaseColor * _EmissiveIntensity;
