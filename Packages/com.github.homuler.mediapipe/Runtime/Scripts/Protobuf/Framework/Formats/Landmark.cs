  ...
  #endif
  {
    ...
    void pb::IBufferMessage.InternalMergeFrom(ref pb::ParseContext input) {
      ...
      while ((tag = input.ReadTag()) != 0) {
        ...
        // 奥山：追加
        // 二つのランドマークの中間のランドマークを作成
        public static NormalizedLandmark Midpoint(NormalizedLandmark a, NormalizedLandmark b)
        {
            NormalizedLandmark midLandmark = new NormalizedLandmark();

            midLandmark.X = (a.X + b.X) * 0.5f;
            midLandmark.Y = (a.Y + b.Y) * 0.5f;
            midLandmark.Z = (a.Z + b.Z) * 0.5f;

            // 両方とも visibility があるなら平均を取る
            if (a.HasVisibility && b.HasVisibility)
            {
                midLandmark.Visibility = (a.Visibility + b.Visibility) * 0.5f;
            }

            // 両方とも presence があるなら平均を取る
            if (a.HasPresence && b.HasPresence)
            {
                midLandmark.Presence = (a.Presence + b.Presence) * 0.5f;
            }
